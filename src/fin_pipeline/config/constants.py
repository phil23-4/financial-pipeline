...
CIK_PATTERN = re.compile(
    r"\b(?:CIK|Central Index Key)(?:[\s#:]|No\.|Number)*0*(\d{1,10})\b", re.IGNORECASE
)
LEI_PATTERN = re.compile(r"\bLEI(?:[\s#:]|Code|Number)*([0-9A-Z]{20})\b", re.IGNORECASE)
ISIN_PATTERN = re.compile(
    r"\bISIN(?:[\s#:]|Code|Number)*([A-Z]{2}[A-Z0-9]{9}\d)\b", re.IGNORECASE
)
CUSIP_PATTERN = re.compile(
    r"\bCUSIP(?:[\s#:]|Code|Number)*([0-9A-Z]{9})\b", re.IGNORECASE
)
SEDOL_PATTERN = re.compile(
    r"\bSEDOL(?:[\s#:]|Code|Number)*([0-9A-Z]{7})\b", re.IGNORECASE
)
